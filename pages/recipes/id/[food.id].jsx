...
const FoodRecipes = () => {
      ...
      const fetchFoods = async () => {
        try {
          const res = await axios.get(
            `https://api-bootcamp.do.dibimbing.id/api/v1/foods/${FOOD_ID}`,
            {
              headers: {
          ...
          console.error(
            "Terjadi kesalahan saat memanggil data makanan:",
            error
          );
        }
      };
  ...
  async function GetReview() {
    try {
      const res = await axios.get(
        `https://api-bootcamp.do.dibimbing.id/api/v1/food-rating/${FOOD_ID}`,
        {
          headers: {
            apiKey: `w05KkI9AWhKxzvPFtXotUva-`,
            "Content-Type": "application/json",
